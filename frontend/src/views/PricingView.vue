...
<template>
  <div class="content-shell">
    <PageHeader title="费用规则" :subtitle="data?.active_version ? `${data.active_version.name} · ${data.active_version.source}` : '当前活动价格版本'">
      <template #actions>
    ...
    <LoadingState :loading="loading" :error="error" :empty="!data" @retry="load">
      <v-alert v-if="data?.active_version?.unpriced_events" type="warning" variant="tonal" border="start" class="mb-4">
        当前价格版本有 {{ number(data.active_version.unpriced_events) }} 条未计价事件。
      </v-alert>
      ...
      <div class="two-column">
        <section class="section-band">
          <div class="section-band__head"><div><h2>梯度计费</h2><p>先按用户聚合各 Key，再应用梯度</p></div></div>
        ...
        <section class="section-band">
          <div class="section-band__head"><div><h2>资源池固定成本</h2><p>{{ data?.billing?.cycle?.name || '未选择账期' }}</p></div></div>
          <div class="section-band__body section-band__body--flush">
            <v-table density="compact">
      ...
      <section class="section-band mt-4">
        <div class="section-band__head">
          <div><h2>模型价格</h2><p>单位：USD / 1M tokens · 激活 {{ dateTime(data?.active_version?.activated_at) }}</p></div>
      ...
      <section class="section-band">
        ...
        <div class="section-band__body semantics-grid">
          ...
          <div><span>未归属 Key</span><strong>不计费</strong></div>
...
<style scoped>
.semantics-grid { display: grid; grid-template-columns: repeat(5, minmax(0, 1fr)); gap: 1px; background: #dfe5e2; padding: 1px; }
